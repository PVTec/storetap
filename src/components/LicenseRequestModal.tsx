...
export default function LicenseRequestModal({ isOpen, onClose, tier }: LicenseRequestModalProps) {
  const displayTier = tier === 'Basic' ? 'Starter' : tier === 'Standard' ? 'Growth' : tier === 'Pro' ? 'Business Pro' : tier;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#0c0c0e] border border-zinc-800 rounded-2xl w-full max-w-md relative shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
        <div className="p-6 border-b border-zinc-800/80 flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-lg font-bold text-white">Get {displayTier} License</h2>
        ...
        {isSuccess ? (
          ...
          <>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="bg-teal-500/10 border border-teal-500/20 rounded-lg p-4 space-y-2 mb-2">
                <div className="bg-teal-500/5 border border-teal-500/20 rounded-xl overflow-hidden mb-4">
                  <div className="p-4 space-y-3">
                    <div className="flex items-center gap-2 mb-2">
                      <svg className="w-4 h-4 text-teal-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      <p className="text-xs text-teal-400 font-bold tracking-wide uppercase">Provider Verification</p>
                    </div>
                    <p className="text-xs text-zinc-400 leading-relaxed">
                      This form submits a request for the {displayTier} License — {tier === 'Basic' ? '₱149 for 30 days' : tier === 'Standard' ? '₱499 for 90 days' : '₱1,499 for 150 days'}. It is not an immediate payment. A StoreTap provider will contact you using the information below to verify your request, confirm your store details, explain the activation process, and prepare the system. <strong className="text-zinc-300 font-semibold">Do not send payment until the provider confirms that your license is ready for activation.</strong>
                    </p>
                    <p className="text-xs text-zinc-400 leading-relaxed">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-teal-500"
              ...
              <div>
                ...
                <input 
                  ...
                  className={`w-full bg-zinc-900 border border-zinc-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-teal-500 ${userLogged ? 'opacity-60 cursor-not-allowed' : ''}`}
              ...
              <div className="flex items-start gap-3 mt-4">
                ...
                <label htmlFor="terms" className="text-xs text-zinc-400 cursor-pointer leading-relaxed">
                  I accept the StoreTap <a href="/terms" target="_blank" className="text-teal-400 hover:underline">Terms of Service</a> and Privacy Policy.
                </label>
              </div>
              ...
              <button 
                ...
                className="w-full bg-teal-500 hover:bg-teal-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3 rounded-lg transition-colors mt-6"
              >
